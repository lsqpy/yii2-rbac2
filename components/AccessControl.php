...
namespace adm\rbac2\components;
...
class AccessControl extends \yii\base\ActionFilter
{
    ...
    public function beforeAction($action)
    {
        ...
        $user = $this->getUser();
        if ($user->can('/' . $actionId, Yii::$app->getRequest()->get(), $user)) {
            return true;
        }
    ...
    protected function isActive($action)
    {
        ...
        //允许访问的用户
        foreach($this->allowUser as $username){
            if(!empty($user->identity->username) && $user->identity->username == $username){
                return false;
            }
        }
